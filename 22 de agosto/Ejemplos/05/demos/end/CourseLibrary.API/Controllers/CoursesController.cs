...
public class CoursesController : ControllerBase
{
        ...
        JsonPatchDocument<CourseForUpdateDto> patchDocument)
    {
        ...
        //+1-Comprobar que el registro existe
        if (!await _courseLibraryRepository.AuthorExistsAsync(authorId))
        {
            return NotFound();
        }

        //+2-Obtener registro ENTIDAD
        var courseForAuthorFromRepo = await _courseLibraryRepository
            .GetCourseAsync(authorId, courseId);
        ...
        //+3-Crear copia del registro para aplicar los cambios
        //+De entidad a DTO
        CourseForUpdateDto courseToPatch = _mapper.Map<CourseForUpdateDto>(
            courseForAuthorFromRepo);

        //+4-Aplicamos los n cambios descritos en el JsonPatchDocument
        //+DTO para actualizar
        patchDocument.ApplyTo(courseToPatch, ModelState);


        //+5-Convertimos a ENTIDAD
        //+DTO para actualizar ENTIDAD
     

        _mapper.Map(courseToPatch, courseForAuthorFromRepo);

        //+6 Actualizar mediante repositorio
