...
namespace Acme.Biz
{
    ...
    public class Vendor
    {
        ...
        {
            //+Validaciones al inicio. En clean code se llama:  Falla rápida
            ...
            var orderTextBuilder = new StringBuilder("Order from Acme, Inc" +
                            ...
                            System.Environment.NewLine +
                            "Quantity: " + quantity); //String builder formula recomendada para trabajar con cadenas.
            ...
            var orderText = orderTextBuilder.ToString(); //Crear cadena nada mas

            var emailService = new EmailService();
            var confirmation = emailService.SendMessage("New Order", orderText,
                                                                     this.Email); //Mandamos correo
            if (confirmation.StartsWith("Message sent:"))
            {
                success = true;
            }
            var operationResult = new OperationResult<bool>(success, orderText); //Si todo va bien, devolvemos el pedido diciendo además que todo ha ido bien
